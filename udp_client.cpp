...
void udp_client::handle_recevie(const boost::system::error_code &ec, std::size_t len)
{
	...
	udp::endpoint *request_peer = new udp::endpoint(boost::asio::ip::address::from_string(peer_ip), std::atoi(peer_port));
        ...
        _sock.send_to(boost::asio::buffer("Sender peer connection complete."), *request_peer);
        boost::thread(boost::bind(&udp_client::p2p_send, this, &_sock, request_peer));
	_sock.send_to(boost::asio::buffer("Sender peer connection complete.1"), *request_peer);
	_sock.send_to(boost::asio::buffer("Sender peer connection complete.2"), *request_peer);
	_sock.send_to(boost::asio::buffer("Sender peer connection complete.3"), *request_peer);
        p2p_receive(_sock, *request_peer);
	std::cout << "PUNCH_SUCCESS end" << std::endl;        
    }
	...
	_sock.send_to(boost::asio::buffer("Receiver peer connection complete."), *request_peer);
        boost::thread(boost::bind(&udp_client::p2p_send, this, &_sock, request_peer));
	_sock.send_to(boost::asio::buffer("Receiver peer connection complete.1"), *request_peer);
	_sock.send_to(boost::asio::buffer("Receiver peer connection complete.2"), *request_peer);
	_sock.send_to(boost::asio::buffer("Receiver peer connection complete.3"), *request_peer);
        p2p_receive(_sock, *request_peer);
	std::cout << "PUNCH_REQUEST end" << std::endl;
    }
    else
    {
        std::cout << receive_message << std::endl;
